...
namespace ComputationalGraph
{
    public class NeuronNode
    {
        ...
        public double forward(List<double> x)
        {
			...
			{
				forSum.Add(this.multiplyNodes[i].forward(new List<double> { inputs[i], multiplyNodes[i].x[1] }));
            }
        ...
        public List<double> backward(List<double> dz)
        {
			...
			var backward_signal_list = this.sumNode.backward(backward_signal);
			
			for (int i = 0; i < backward_signal_list.Count(); i++) 
			{
				dw.Add(this.multiplyNodes[i].backward(backward_signal_list[i])[1]);
			}
